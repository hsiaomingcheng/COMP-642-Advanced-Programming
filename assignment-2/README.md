...
**How to run:** from this folder, run `python3 main.py`. It needs Python 3.10 or newer (the type hints use `X | None`); it was developed on Python 3.14.
`main.py` saves its data to `demo_data.pkl` (next to `nurserySystem.py`) and deletes any old copy when it starts, so it can be run repeatedly.
...
- **`Plant`, `Customer` and `Payment` are abstract** because a plant that is none of the four types, a customer that is none of the three types, and a payment that is neither credit nor debit do not exist in Brent's notes, so they can never be created directly.
- **A member is abstract only when there is no sensible shared default.** `category`, `customer_type` and `payment_type` differ for every subclass. Where most subclasses share a value, the base class supplies it and only the exceptions override it (`TreesAndShrubs` and `Perennial` do not write `unit_label` at all).
...
- `Order` and `NurserySystem` call the same methods on every plant, customer and payment (`discount_rate`, `check_can_place_order`, `check_can_collect`, `unit_label`, `surcharge`) and never check which subclass they are dealing with; each subclass answers in its own way.

## 2. How the design supports Brent's follow-up notes
...
| An order can have several plants, each with its own quantity and cost | `Order` holds `OrderItem`s; each has a plant, a quantity and `get_cost()`. The order total is the sum of the item costs. Adding a plant that is already on the order increases that item's quantity, so each plant appears once. Totals are calculated every time from the items, never stored |
...
| Payment record: id, amount, customer, order, date | `Payment` attributes (the date is validated as DD-MM-YYYY) |
...
## 3. Brent's original notes (Assignment 1), and what changed

| Original requirement | Where it is handled now |
|---|---|
| Know the stock of each plant; refuse an order if there is not enough; check stock before ordering | `Plant.stock_level`, `stock_level_check`; `stock_level_check_and_buy` raises `InsufficientStockError`; `Order.place()` checks every item before changing anything, so a rejected order changes nothing |
...
10. **All calculated money values (item cost, order total, surcharge, balance) are rounded to two decimals,** so percentages such as 1%, 5% and 1.5% cannot leave tiny floating point differences that would stop an order from being fully paid.
11. **Credit card details:** the card number must be exactly 16 digits (stored as text so a leading zero is kept) and the expiry date is written `MM/YY`. Debit card bank names must not be blank. Only the format of the expiry date is checked, so an expired card is not rejected.
